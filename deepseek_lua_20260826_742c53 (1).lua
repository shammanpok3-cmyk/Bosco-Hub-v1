--[[
    Bosco Hub V1 — LocalScript (All Devices)
    Made by Bosco
    Updated with Triggerbot, Responsive UI, and Mobile Support
]]
...
-- Cloneref for security
local repS = cloneref(ReplicatedStorage)
local plrs = cloneref(Players)
local runS = cloneref(RunService)
local ws = cloneref(Workspace)
local uis = cloneref(UIS)
...
local SpectateController = require(LP.PlayerScripts.Controllers:WaitForChild("SpectateController"))
...
-- ============================================
Features.triggerbotEnabled = false
Features.triggerbotSettings = {
    FireRate = 0.1,
    MaxDistance = 500,
    CheckDeflection = true,
    CheckSpawnShield = true,
...
function Triggerbot.UpdateDeflection()
    ...
    for _, fighterObj in FighterController.Objects do
        ...
        if not fighterObj.Entity or not fighterObj.Entity:IsAlive() or fighterObj:Get("IsSpectating") then
            Triggerbot.State.DeflectingPlayers[player] = false
            continue
        end
        ...
        Triggerbot.State.DeflectingPlayers[player] = isDeflecting
    end
end

function Triggerbot.IsEnemy(player)
    if player == LP then return false end
    local duel = SpectateController.CurrentDuelSubject
    local localDueler = duel and duel:GetDueler(LP)
    ...
    local lTeam = LP:GetAttribute("TeamID")
...
function Triggerbot.GetTargetUnderCrosshair()
    ...
    local raycastResult = ws:Raycast(camera.CFrame.Position, camera.CFrame.LookVector * Features.triggerbotSettings.MaxDistance, raycastParams)
    if not raycastResult then return nil, nil end
    
    local hitInstance = raycastResult.Instance
    local hitCharacter = hitInstance.Parent
    
    local targetPlayer = plrs:GetPlayerFromCharacter(hitCharacter)
    if not targetPlayer then return nil, nil end
    
    if Features.triggerbotSettings.TeamCheck and not Triggerbot.IsEnemy(targetPlayer) then
        return nil, nil
    end
    
    if Features.triggerbotSettings.CheckDeflection and Triggerbot.State.DeflectingPlayers[targetPlayer] then
        return nil, nil
    end
    
    if Features.triggerbotSettings.CheckSpawnShield then
        local forceField = hitCharacter:FindFirstChildOfClass("ForceField")
        if forceField and forceField.Visible then
            return nil, nil
        end
    end
    ...
    return targetPlayer, hitInstance
end

function Triggerbot.Fire(targetPlayer, targetPart)
    if tick() - Triggerbot.State.LastFireTime < Features.triggerbotSettings.FireRate then
        return false
    end
    
    Triggerbot.State.LastFireTime = tick()
    ...
    local camera = ws.CurrentCamera
    local aimCF = camera.CFrame
    local targetCF = targetPart.CFrame
    local objSpaceHeadOffset = targetPart.CFrame:ToObjectSpace(CFrame.new(targetPart.Position))
...
local function triggerbotUpdate()
    if not Features.triggerbotEnabled then 
        Triggerbot.State.IsFiring = false
        return 
    end
    
    Triggerbot.UpdateDeflection()
    ...
    local targetPlayer, targetPart = Triggerbot.GetTargetUnderCrosshair()
    
    if targetPlayer and targetPart then
        Triggerbot.State.IsFiring = true
        Triggerbot.Fire(targetPlayer, targetPart)
    else
        Triggerbot.State.IsFiring = false
    end
end

function Features.setTriggerbot(on) 
    Features.triggerbotEnabled = on
    if on then
        if not triggerbotLoop then
            triggerbotLoop = RunService.Heartbeat:Connect(triggerbotUpdate)
        end
    else
...
Players.PlayerRemoving:Connect(function(player)
    Triggerbot.State.DeflectingPlayers[player] = nil
end)
...
local function setupSilentAim()
    ...
    pcall(function()
        local Utility = require(repS.Modules.Utility)
...
-- ============================================
local Keybinds = { 
    triggerbot={key=Enum.KeyCode.E, holdMode=false, label="Triggerbot"},
    silentAim={key=Enum.KeyCode.T, holdMode=false, label="Silent Aim"}, 
    esp={key=Enum.KeyCode.Z, holdMode=false, label="ESP"}, 
    infiniteJump={key=Enum.KeyCode.X, holdMode=false, label="Inf Jump"}, 
    flick={key=Enum.KeyCode.F, holdMode=false, label="Flick"}, 
    unlockAll={key=Enum.KeyCode.U, holdMode=false, label="Unlock All"} 
}
...
local function saveConfig(name, overwrite)
    ...
    configs[name] = { 
        triggerbot=Features.triggerbotEnabled, 
        triggerbotSettings=Features.triggerbotSettings,
        silentAim=Features.silentAimEnabled, 
        silentAimSettings=Features.silentAimSettings, 
        esp=Features.espEnabled, 
        espSettings=Features.espSettings, 
        infiniteJump=Features.infiniteJumpEnabled, 
        flick=Features.flickEnabled, 
        flickSettings=Features.flickSettings, 
        unlockAll=Features.unlockAllEnabled, 
        staffDetector=Features.staffDetectorEnabled, 
        stretchRes=Features.stretchResEnabled, 
        stretchResValue=Features.stretchResValue, 
        keybinds=keybindsCopy 
    }
...
local function loadConfig(name)
    ...
    if data.triggerbot ~= nil then Features.setTriggerbot(data.triggerbot) end
    if data.triggerbotSettings then for k, v in pairs(data.triggerbotSettings) do Features.triggerbotSettings[k] = v end end
...
function ResponsiveUI.DetectDevice()
    local viewportSize = Camera.ViewportSize
    ResponsiveUI.State.ScreenSize = viewportSize
    local width = viewportSize.X
    
    if width <= 800 or (UIS.TouchEnabled and width <= 1024) then
        ResponsiveUI.State.Device = "Mobile"
    elseif width <= 1200 then
        ResponsiveUI.State.Device = "Tablet"
    else
        ResponsiveUI.State.Device = "PC"
    end
    
    ResponsiveUI.State.IsMobile = (ResponsiveUI.State.Device == "Mobile")
    local deviceConfig = ResponsiveUI.Config.Devices[ResponsiveUI.State.Device]
    ResponsiveUI.State.Scale = deviceConfig.UIScale
    
    return ResponsiveUI.State.Device
end

ResponsiveUI.DetectDevice()

Camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
    ResponsiveUI.DetectDevice()
    if root then
        local deviceConfig = ResponsiveUI.Config.Devices[ResponsiveUI.State.Device]
        root.Size = UDim2.fromOffset(deviceConfig.RootWidth, deviceConfig.RootHeight)
        sidebar.Size = UDim2.new(0, deviceConfig.SidebarWidth, 1, 0)
        main.Size = UDim2.new(1, -deviceConfig.SidebarWidth, 1, 0)
        main.Position = UDim2.fromOffset(deviceConfig.SidebarWidth, 0)
    end
end)
...
local deviceConfig = ResponsiveUI.Config.Devices[ResponsiveUI.State.Device]
local root=Instance.new("Frame");root.Name="Root";root.BackgroundColor3=C.bg;root.BorderSizePixel=0;root.Size=UDim2.fromOffset(deviceConfig.RootWidth,deviceConfig.RootHeight)
...
local sidebar=Instance.new("Frame");sidebar.BackgroundColor3=C.sidebarBg;sidebar.BorderSizePixel=0;sidebar.Size=UDim2.new(0,deviceConfig.SidebarWidth,1,0);sidebar.ZIndex=5;sidebar.Parent=body
...
local main=Instance.new("Frame");main.Name="Main";main.BackgroundColor3=C.panel;main.BorderSizePixel=0;main.Size=UDim2.new(1,-deviceConfig.SidebarWidth,1,0);main.Position=UDim2.fromOffset(deviceConfig.SidebarWidth,0);main.ZIndex=4;main.Parent=body
...
local function renderTriggerbot()
    clearContent();local lo=1;local s=Features.triggerbotSettings
    secHead("🎯 Triggerbot",lo);lo+=1
    createToggle({title="Enable Triggerbot",desc="Auto fire when crosshair on enemy",layoutOrder=lo,default=Features.triggerbotEnabled,onChange=function(on)Features.setTriggerbot(on)end});lo+=1
    createDualSlider({title="Fire Rate",desc="Delay between shots",layoutOrder=lo,default=s.FireRate,min=0,max=1,onChange=function(v)s.FireRate=v end});lo+=1
    createDualSlider({title="Max Distance",desc="Maximum fire distance",layoutOrder=lo,default=s.MaxDistance,min=50,max=1000,onChange=function(v)s.MaxDistance=v end});lo+=1
    createToggle({title="Check Deflection",desc="Don't shoot deflecting Katana",layoutOrder=lo,default=s.CheckDeflection,onChange=function(on)s.CheckDeflection=on end});lo+=1
    createToggle({title="Check Spawn Shield",desc="Don't shoot spawn shield",layoutOrder=lo,default=s.CheckSpawnShield,onChange=function(on)s.CheckSpawnShield=on end});lo+=1
    createToggle({title="Team Check",desc="Don't shoot teammates",layoutOrder=lo,default=s.TeamCheck,onChange=function(on)s.TeamCheck=on end});lo+=1
end

local function renderSilentAim()
    clearContent();local lo=1;local s=Features.silentAimSettings
    secHead("🔇 Silent Aim",lo);lo+=1
    createToggle({title="Enable Silent Aim",desc="Hit targets without aiming",layoutOrder=lo,default=Features.silentAimEnabled,onChange=function(on)Features.setSilentAim(on)end});lo+=1
...
local function renderStretchRes()
    clearContent();local lo=1
...
local function renderKeybinds()
    ...
    createActionRow({title="Reset All Keybinds",desc="Restore defaults",layoutOrder=lo,buttonText="↺ Reset",onClick=function()
        ...
        for k, v in pairs(defaults) do
            Keybinds[k] = {key=v.key, holdMode=v.holdMode, label=v.label}
        end
...
-- NAVIGATION
mkNavHdr("🎯 COMBAT",1)
mkNav("Triggerbot",2,renderTriggerbot)
...
mkSep(sbScroll,17)
mkNavHdr("📐 VISUALS+",18)
mkNav("Stretch Res",19,renderStretchRes)
mkSep(sbScroll,20)
...
renderTriggerbot()
if navBtns["Triggerbot"]then setNavActive(navBtns["Triggerbot"])end

-- MOBILE BUTTON
local mobileButton = nil
if ResponsiveUI.State.IsMobile then
    ...
    mobileButton.BackgroundColor3 = Color3.fromRGB(140, 30, 255)
    mobileButton.Image = "rbxassetid://134381155775118"
    ...
    mStroke.Color = Color3.fromRGB(180, 80, 255)
    mStroke.Thickness = 2
...
UIS.InputBegan:Connect(function(input,gp)
    ...
    for id,bind in pairs(Keybinds)do
        if input.KeyCode==bind.key and not bind.holdMode then
            if id=="triggerbot"then Features.setTriggerbot(not Features.triggerbotEnabled)end
...
print("Bosco Hub V1 loaded! (All Devices)")
