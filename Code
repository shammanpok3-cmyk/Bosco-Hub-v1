...
local function renderFun()
    ...
    for _,device in ipairs(devices)do
        createToggle({title=device[1],desc="Spoof as "..device[1],layoutOrder=lo,default=autoSpoofDevice==device[2],onChange=function(on)if on then autoSpoofDevice=device[2];spoofDevice(device[2]);showToast("Spoofed: "..device[1])else autoSpoofDevice=nil end end})
        lo+=1
    end
    mkSep(mainScroll,lo);lo+=1
    secHead("📐 Stretch Resolution",lo);lo+=1
    createToggle({title="Enable Stretch Res",desc="Makes enemies wider",layoutOrder=lo,default=Features.stretchResEnabled,onChange=function(on)Features.setStretchRes(on)end});lo+=1
...
mkSep(sbScroll,17)
mkNavHdr("🎭 FUN",18)
mkNav("Device Spoofer",19,renderFun)
mkSep(sbScroll,20)
